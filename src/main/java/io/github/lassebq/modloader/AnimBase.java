...
public abstract class AnimBase extends TextureAtlas {
    ...
    public AnimBase(int spriteID, String spritePath) {
        super(spriteID);
        this.size = (int)Math.sqrt((double)(this.buffer.length / 4));
        ...
        try {
            ...
            if (spritePath.isEmpty()) {
                bufImage = ImageIO.read(Minecraft.class.getResource(this.type == 0 ? "/terrain.png" : "/gui/items.png"));
    ...
    protected void copyFrameToArray() {
        for(int y = 0; y < this.size; ++y) {
            for(int x = 0; x < this.size; ++x) {
                ...
                this.buffer[index * 4 + 0] = (byte)(this.frame[x][y] >> 16 & 255);
                this.buffer[index * 4 + 1] = (byte)(this.frame[x][y] >> 8 & 255);
                this.buffer[index * 4 + 2] = (byte)(this.frame[x][y] & 255);
                this.buffer[index * 4 + 3] = (byte)(this.frame[x][y] >> 24 & 255);
            }
        }
